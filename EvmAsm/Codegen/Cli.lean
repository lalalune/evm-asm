  ...
  | .ok opts => do
      ...
      | .listTestCases => do
          ...
          for tc in Tests.opcodeTestCases do
            ...
            IO.println
              s!"{tc.name}\t{tc.expectedOutHex}\t{tc.bytecode}\t{tc.calldata}\t{tc.storage}\t{tc.blobBaseFee}\t{tc.blobHashes}\t{tc.expectedHaltKind}\t{tc.expectedPersistentLogLength}\t{tc.expectedTransientLogLength}\t{tc.expectedPostStorage}\t{tc.expectedEventLogCount}\t{tc.expectedEventLogFirst}"
          return 0
      | .program => do
