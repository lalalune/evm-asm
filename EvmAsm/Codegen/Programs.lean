...
/-! ## File-size guard
    ...
    Hard cap on `Programs.lean` to prevent uncontrolled growth. The
    file has already been split twice (PR-#5870 and PR-#5900 carved
    out `Evm.lean`, `HashBridge.lean`, `Ssz.lean`, `RlpRead.lean`,
    and `Mpt.lean`); when this guard trips, do the same again —
    pick a cluster of `*Function` / `zisk*` defs, lift them into a
    new submodule under `EvmAsm/Codegen/Programs/`, and have
    `Programs.lean` import it.

    The cap is line-based and intentionally generous: roughly 20 %
    headroom over the current size at the time of writing. If you
    have a legitimate reason to bump it (e.g., shipping a single
    very-large primitive that resists factoring), edit the literal
    below and document it in the commit message.
...
#eval show IO Unit from do
  ...
  if lineCount > softCap then
    throw <| IO.userError <|
      ...
      "established pattern), or bump the cap in the guard at the " ++
      "bottom of Programs.lean with a justifying commit message."
