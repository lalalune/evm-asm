...
/-- M9 signed division handlers: SDIV (0x05) and SMOD (0x07).
    ...
    **Trampoline pattern:**
    ...
    Both `evm_sdiv` and `evm_smod` now use the corrected v4 callable
    wrappers (`evm_div_callable_v4` / `evm_mod_callable_v4`), so the
    runtime-dispatcher handlers exercise the migrated signed opcode
