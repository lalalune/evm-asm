...
/-- Exactly 108 opcode bytes are claimed by `tinyInterpRegistry` today.
    Update this number when wiring new opcodes (EXP, etc.). -/
theorem tinyInterpRegistry_wired_opcode_count :
    (tinyInterpRegistry.flatMap (·.opcodes)).length = 108 := by
  decide

/-- Exactly 108 bytes in `[0, 255]` route to a registered handler;
    the remaining 148 fall through to `h_invalid`. -/
theorem jumpTable_non_invalid_count :
    ((List.range 256).filter
      (fun b => jumpTargetLabel tinyInterpRegistry b ≠ "h_invalid")).length = 108 := by
  set_option maxRecDepth 2048 in decide
