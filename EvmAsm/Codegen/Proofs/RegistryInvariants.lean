...
/-- Exactly 91 opcode bytes are claimed by `tinyInterpRegistry` today.
    Update this number when wiring new opcodes (M11, EXP, etc.). -/
theorem tinyInterpRegistry_wired_opcode_count :
    (tinyInterpRegistry.flatMap (·.opcodes)).length = 91 := by
  decide

/-- Exactly 91 bytes in `[0, 255]` route to a registered handler;
    the remaining 165 fall through to `h_invalid`. -/
