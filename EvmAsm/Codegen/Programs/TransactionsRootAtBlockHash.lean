/-
  EvmAsm.Codegen.Programs.TransactionsRootAtBlockHash

  Hash-keyed `header.transactions_root` extractor (RLP
  field 4, 32 bytes). Mirror of the number-keyed
  `transactions_root_at_block_number` probe but takes a
  block_hash key.
...
/-! ## transactions_root_at_block_hash
    ...
    `header.block.transactions_root` (RLP field 4, 32 B).
    ...
    Use cases:
      * Bridge-driven tx-trie attestation: caller has a
        known block_hash + an off-chain claim about the
        tx-root; verify directly.
      * Cross-witness consistency: two witnesses sharing a
        block_hash must agree on transactions_root.
      * Reorg detection.
    ...
    Calling convention (4 args):
      ...
      a0 (output) :
        ...
        2 = matched header transactions_root extraction failed
            (RLP malformed / field 4 size != 32)
-/
def transactionsRootAtBlockHashFunction : String :=
  "transactions_root_at_block_hash:\n" ++
  "  addi sp, sp, -64\n" ++
  "  sd ra,  0(sp)\n" ++
  ...
  "  mv a1, s2\n" ++
  "  mv a2, s0\n" ++
  "  la a3, trbh_match_offset\n" ++
  "  la a4, trbh_match_length\n" ++
  "  jal ra, witness_lookup_by_hash\n" ++
  "  bnez a0, .Ltrbh_no_match\n" ++
  "  la t0, trbh_match_offset\n" ++
  "  ld t1, 0(t0)\n" ++
  "  add s4, s1, t1\n" ++
  "  la t0, trbh_match_length\n" ++
  "  ld s5, 0(t0)\n" ++
  "  mv a0, s4\n" ++
  "  mv a1, s5\n" ++
  "  mv a2, s3\n" ++
...
/-- `zisk_transactions_root_at_block_hash`: probe BuildUnit. -/
def ziskTransactionsRootAtBlockHashPrologue : String :=
  "  li sp, 0xa0050000\n" ++
  "  li t4, 0x40000000\n" ++
  ...
  "  jal ra, transactions_root_at_block_hash\n" ++
  "  li t0, 0xa0010000\n" ++
  "  sd a0, 0(t0)\n" ++
  "  j .Ltrbh_pdone\n" ++
  ...
  transactionsRootAtBlockHashFunction ++ "\n" ++
  ".Ltrbh_pdone:"

def ziskTransactionsRootAtBlockHashDataSection : String :=
  ".section .data\n" ++
  ".balign 32\n" ++
  "zk3_state:\n" ++
  "  .zero 200\n" ++
