...
/-! ## chain_compute_min_gas_limit -- PR-K263
    ...
    header chain. Min counterpart to K262
    chain_compute_max_gas_limit; mirrors K238
    chain_compute_min_gas_used (over field 10).

    Useful for spotting capacity bottlenecks across a chain
    segment.
...
/-! ## chain_compute_total_gas_limit -- PR-K264
    ...
    Sum `gas_limit` (header field 9) across an N-element header
    chain. The sum counterpart to K262/K263 (max/min gas_limit);
    mirrors K196 chain_compute_total_gas_used (over gas_used
    field 10).

    Useful as a chain-segment capacity metric — the cumulative
    cap divided by the actual gas_used sum gives a utilization
    fraction.
...
/-! ## chain_extract_gas_limit_first_last -- PR-K265
    ...
    from an N-element header chain. Gas-limit counterpart to K247
    chain_extract_basefee_first_last and K245
    chain_extract_gas_used_range. Useful for measuring capacity
    drift across a chain segment.
