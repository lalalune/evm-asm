...
/-- Build the all-phases invariant from the Phase-1 invariant plus the Phase-2
...
theorem Div128AllPhasesNoWrapInv.ofPhaseNoWrapInv
    {uHi uLo vTop : Word}
    (h_phase : Div128PhaseNoWrapInv uHi uLo vTop)
    (h_phase2 :
      let dHi := vTop >>> (32 : BitVec 6).toNat
      let dLo := (vTop <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
      let div_un1 := uLo >>> (32 : BitVec 6).toNat
  ...
  simp only [Div128PhaseNoWrapInv] at h_phase
