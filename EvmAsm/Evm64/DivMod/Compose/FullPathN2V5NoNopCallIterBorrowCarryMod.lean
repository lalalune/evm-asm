/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopCallIterBorrowCarryMod

  MOD mirror of `FullPathN2V5NoNopCallIterBorrowCarry`: borrow-dispatched j=2 / j=1
  N2 call iteration bodies over `modCode_noNop_v5`, requiring carry2nz only
  conditionally on the runtime borrow.  Byte-for-byte the DIV proof — delegate
  on borrow to the MOD base call-iter bodies (#7693), inline the SKIP path on
  no-borrow via the MOD call-skip exact-x1 body (#7689).  Brick 8 of the n=2 MOD
  loop body (call iter borrowCarry layer).  Bead `evm-asm-wbc4i.10.3.2.4.5`.
-/
...
/-- j=2 N2 call iteration over `modCode_noNop_v5`, carry2nz required only on the
    runtime-borrow branch. -/
theorem divK_loop_body_n2_call_j2_exact_loopIterScratch_v5_noNop_borrowCarry_modCode (sp base : Word)
  ...
  · exact divK_loop_body_n2_call_j2_exact_loopIterScratch_v5_noNop_modCode
...
/-- j=1 N2 call iteration over `modCode_noNop_v5`, carry2nz required only on the
    runtime-borrow branch. -/
theorem divK_loop_body_n2_call_j1_exact_loopIterScratch_v5_noNop_borrowCarry_modCode (sp base : Word)
    (jOld v5Old v6Old v7Old v10Old v11Old v2Old : Word)
    (v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld raVal : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (halign : ((base + div128CallRetOff) + signExtend12 (0 : BitVec 12)) &&& ~~~(1 : Word) =
  ...
  by_cases hborrow :
      BitVec.ult uTop
        (mulsubN4_c3 (divKTrialCallV5QHat u2 u1 v1) v0 v1 v2 v3 u0 u1 u2 u3)
  · exact divK_loop_body_n2_call_j1_exact_loopIterScratch_v5_noNop_modCode
      sp base jOld v5Old v6Old v7Old v10Old v11Old v2Old
      v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld raVal
      retMem dMem dloMem scratchUn0 scratchMem
      halign hbltu (hcarry2_borrow hborrow)
