/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN1V5Bzero

  v5 bzero DIV code path (b = 0 → quotient 0), over `divCode_noNop_v5`: phaseA
  detects b=0, the BEQ at phaseABeqOff is taken to zeroPath, which writes 0 to the
  output slots.  Mirror of the v4 `evm_div_bzero_spec_within_noNop_v4` with the
  phaseA/zeroPath/beq code subsumptions re-pointed at `divCode_noNop_v5`
...
private theorem divK_phaseA_code_sub_divCode_noNop_v5 {base : Word} :
  ...
  intro a i h
  exact sharedNoNop_v5_b0_div a i (by unfold divK_phaseA_code at h; exact h)

private theorem divK_zeroPath_code_sub_divCode_noNop_v5 {base : Word} :
    ∀ a i, (divK_zeroPath_code (base + zeroPathOff)) a = some i → (divCode_noNop_v5 base) a = some i := by
  intro a i h
  exact sharedNoNop_v5_b10_div a i (by unfold divK_zeroPath_code at h; exact h)
...
theorem evm_div_bzero_spec_within_noNop_v5 (sp base : Word)
  ...
  have hzp := cpsTripleWithin_extend_code divK_zeroPath_code_sub_divCode_noNop_v5
...
/-- v5 no-NOP bzero stack wrapper (evmWordIs form), over `divCode_noNop_v5`. -/
theorem evm_div_bzero_stack_spec_within_noNop_v5 (sp base : Word)
  ...
  have h_raw := evm_div_bzero_spec_within_noNop_v5 sp base
...
/-- v5 full-pre bzero dispatcher, over `divCode_noNop_v5`. -/
theorem evm_div_bzero_stack_spec_within_dispatch_noNop_v5_uni (sp base : Word)
...
/-- The v5 bzero DIV lane, matching `evm_div_stack_spec_unconditional_of_lanes_v5_div`:
    from the NoX1 stack-dispatch precondition (plus the `sp+3936` scratch cell) to
    `divStackDispatchPostV5`, over `divCode_noNop_v5`.  Frames the unused `sp+3936`
...
theorem evm_div_bzero_lane_v5 (sp base : Word) (a b : EvmWord)
  ...
  have hw := cpsTripleWithin_frameR ((sp + signExtend12 3936) ↦ₘ scratchMem) (by pcFree)
    (evm_div_bzero_stack_spec_within_dispatch_noNop_v5_uni sp base a b
