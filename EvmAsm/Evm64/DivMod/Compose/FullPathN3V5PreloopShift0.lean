/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN3V5PreloopShift0

  v5 n=3 shift=0 preloop pieces over `divCode_noNop_v5`.  On the shift=0 branch the
  divisor is already top-bit-aligned (`(clzResult b2).1 = 0`, `b2 ≥ 2^63`), so
  normalization is skipped: phase-C2 takes the BEQ to copyAU.  This file builds the
  n=3 phaseAB-clz + phaseC2(shift=0 taken) segment (`base → copyAUOff`), the n=3
  analog of `evm_div_phaseAB_n2_clz_c2taken_spec_v5_noNop`.  Composes the n=3
  phaseAB clz (`evm_div_phaseAB_n3_clz_spec_within_v5_noNop`) with the shared
  shift=0 phase-C2 branch (`divK_phaseC2_taken_spec_within_v5_noNop`).
...
/-- v5 n=3 phaseAB-clz + phaseC2(shift=0 taken), `base → copyAUOff`, over
    `divCode_noNop_v5`.  Shift=0 branch (`(clzResult b2).1 = 0`): the divisor is
    already aligned, so phase-C2 takes the BEQ to copyAU. -/
theorem evm_div_phaseAB_n3_clz_c2taken_spec_v5_noNop (sp base : Word)
  ...
  have hABCLZf := cpsTripleWithin_frameR
    ...
    (by pcFree) hABCLZ
  have hC2 := divK_phaseC2_taken_spec_within_v5_noNop sp (clzResult b2).1
...
/-- Full n=3 shift=0 preloop, `base → loopBodyOff`, over `divCode_noNop_v5`.
    Shift=0 analog of the n=3 preloop: on the shift=0 branch
    (`(clzResult b2).1 = 0`) the dividend is copied verbatim (u-cells ← a-cells,
    `u[4] = 0`) instead of normalized.  Loop counter `x5 ← 3`, `x9 ← 4 − 3`. -/
theorem evm_div_n3_to_loopSetup_shift0_spec_v5_noNop (sp base : Word)
    ...
    cpsTripleWithin ((8 + 21 + 24 + 4) + 13) base (base + loopBodyOff)
      ...
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ (3 : Word)) **
       ...
       ((sp + signExtend12 4000) ↦ₘ (0 : Word)) ** ((sp + signExtend12 3984) ↦ₘ (3 : Word)) **
       ((sp + signExtend12 3992) ↦ₘ (clzResult b2).1)) := by
  have hC2 := evm_div_phaseAB_n3_clz_c2taken_spec_v5_noNop sp base b0 b1 b2 b3
    v2 v5 v6 v7 v10 q0 q1 q2 q3 u5 u6 u7 nMem shiftMem hbnz hb3z hb2nz hshift_z
  have hC2f := cpsTripleWithin_frameR
    ((.x9 ↦ᵣ signExtend12 (4 : BitVec 12) - (4 : Word)) **
     ((sp + 0) ↦ₘ a0) ** ((sp + 8) ↦ₘ a1) **
     ((sp + 16) ↦ₘ a2) ** ((sp + 24) ↦ₘ a3) **
  ...
  have hSeg := divK_copyAU_loopSetup_shift0_spec_v5_noNop sp base
    a0 a1 a2 a3 u0Old u1Old u2Old u3Old u4Old (clzResult b2).2
    (signExtend12 (4 : BitVec 12) - (4 : Word)) (3 : Word) (by decide)
