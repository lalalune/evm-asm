/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopLoopSelectedBorrowCarry

  Instantiated borrow-dispatched n=2 v5 loop (register-slot form): re-exposes the
  borrow-dispatched unified loop (#7449) with the loop-setup register slots named,
  taking `loopN2SelectedBorrowCarryV5` (satisfiable from shape).  Mirror of the
...
theorem evm_div_n2_loop_unified_inst_noNop_exact_x1_v5_borrowCarry
  ...
  (simp only at hbltu_0 hbltu_1 hbltu_2;
   exact divK_loop_n2_unified_from_source_exact_loopIterScratch_v5_noNop_borrowCarry
     _ _ _ sp base
