/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN4V5NoNopPreloop

  The n=4 v5 preloop pieces over `divCode_noNop_v5`, mirroring the n=3 v5 preloop
  (FullPathN3V5NoNopPreloop).  First piece: PhaseB(n=4) — the divisor-shape dispatch
  for the n=4 branch (`b3 ≠ 0` ⟹ n = 4, BNE taken).  Direct mirror of the noNop
  `evm_div_phaseB_n4_spec_within_noNop` (PhaseABNoNop) with the v5 code-subset
  lemmas (`*_code_sub_divCode_noNop_v5`).  Start of the n=4 v5 code-path assembly
...
/-- PhaseB(n=4) over `divCode_noNop_v5`: the `b3 ≠ 0` dispatch branch (n = 4). -/
theorem evm_div_phaseB_n4_spec_within_v5_noNop (sp base : Word)
  ...
  have haddi := cpsTripleWithin_extend_code addi_x5_singleton_sub_divCode_noNop_v5 haddi_raw
  ...
  have hbne := cpsTripleWithin_extend_code bne_x10_singleton_sub_divCode_noNop_v5 hbne_clean
...
/-- PhaseAB(n=4) + CLZ(b3) over `divCode_noNop_v5`: dispatch to n=4 and compute the
    normalization shift of the top limb `b3`.  n=4 analog of
    `evm_div_phaseAB_n3_clz_spec_within_v5_noNop`. -/
theorem evm_div_phaseAB_n4_clz_spec_within_v5_noNop (sp base : Word)
  ...
  have hAf := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) **
     ...
     ((sp + signExtend12 4000) ↦ₘ u7) ** ((sp + signExtend12 3984) ↦ₘ nMem))
    (by pcFree) hA
  have hB := evm_div_phaseB_n4_spec_within_v5_noNop sp base b1 b2 b3
  ...
  have hCLZ := divK_clz_spec_within_v5_noNop b3 b1 b2 base
...
/-- PhaseAB(n=4) + CLZ(b3) + PhaseC2(ntaken) + NormB over `divCode_noNop_v5`:
