/-
  EvmAsm.Evm64.DivMod.Compose.NormBV5

  v5 normalize-B brick (21 steps, normBOff → normAOff) over `divCode_noNop_v5`.
  ...
  are extended to the v5 code via the v5 block subsumption `sharedNoNop_v5_b4_div`.
...
/-- The normB block instructions are subsumed by `divCode_noNop_v5`. -/
...
private theorem divK_normB_half1_within_v5_noNop
  ...
  have hm1e := cpsTripleWithin_extend_code (hmono := fun a i h =>
    divK_normB_code_sub_divCode_noNop_v5 a i
...
private theorem divK_normB_half2_within_v5_noNop
    ...
    cpsTripleWithin 9 (base + normBOff + 48) (base + normAOff) (divCode_noNop_v5 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ b2') ** (.x7 ↦ᵣ (b1 >>> (antiShift.toNat % 64))) **
       (.x6 ↦ᵣ shift) ** (.x2 ↦ᵣ antiShift) **
       ((sp + 32) ↦ₘ b0) ** ((sp + 40) ↦ₘ b1) **
       ((sp + 48) ↦ₘ b2') ** ((sp + 56) ↦ₘ b3'))
...
/-- v5 normalize-B full brick, over `divCode_noNop_v5`.  Mirror of the v4 analog. -/
theorem divK_normB_full_spec_within_v5_noNop
    (sp b0 b1 b2 b3 v5 v7 shift antiShift : Word) (base : Word) :
    cpsTripleWithin 21 (base + normBOff) (base + normAOff) (divCode_noNop_v5 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) ** (.x7 ↦ᵣ v7) **
       (.x6 ↦ᵣ shift) ** (.x2 ↦ᵣ antiShift) **
       ((sp + 32) ↦ₘ b0) ** ((sp + 40) ↦ₘ b1) **
       ((sp + 48) ↦ₘ b2) ** ((sp + 56) ↦ₘ b3))
