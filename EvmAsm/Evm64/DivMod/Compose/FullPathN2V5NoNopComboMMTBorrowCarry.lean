/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboMMTBorrowCarry

  Borrow-dispatched variant of the n=2 max×max×call source path (ComboMMT):
  each digit's carry required only conditionally on its runtime borrow.
  Composes the max×max prefix borrowCarry (#7440) + j=0 call exit body
  borrowCarry (#7432).
-/
...
theorem divK_loop_n2_max_max_call_from_source_exact_loopIterScratch_v5_noNop_borrowCarry
    (sp base : Word)
