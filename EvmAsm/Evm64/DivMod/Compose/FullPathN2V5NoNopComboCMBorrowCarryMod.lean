/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboCMBorrowCarryMod

  MOD mirror of `FullPathN2V5NoNopComboCMBorrowCarry`: the borrow-dispatched n=2
  call×max two-digit prefix over `modCode_noNop_v5`.  Byte-for-byte the DIV proof
  — chain the MOD j=2 call source borrowCarry (#7696) with the MOD j=1 max iter
  borrowCarry body (#7691) via the code-agnostic call→max source-pre bridge
  `loopIterPostN2CallScratchNoX1_j2_to_max_j1_pre`.  Brick 13 of the n=2 MOD loop
  body (combo layer — call×max 2-combo; last of the four 2-digit prefixes).
...
/-- Full n=2 call×max prefix over `modCode_noNop_v5`, carry required only on each
    digit's runtime-borrow branch. -/
theorem divK_loop_n2_call_max_from_source_exact_loopIterScratch_v5_noNop_borrowCarry_modCode
    (sp base : Word)
