/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopSource

  Source-level v5/no-NOP wrappers for the n=2 DIV loop: the first-iteration
  (j=2) call and max bodies from the callable-ready loop source, preserving
  concrete `x1` and carrying the j=1/j=0 source atoms as a frame.  Mirror of the
  j=2 source theorems in `FullPathN2V4NoNopSource`, swapping the iter bodies
  (v4→v5, #7391/#7392), the code surface, and the v5 trial accessors in the
  call post; the loop pre (`loopN2PreWithScratchV4NoX1`) and the pre-bridge
  lemmas are code-agnostic and reused from the v4 wrappers.
-/
