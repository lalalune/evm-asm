/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboCCMBorrowCarryMod

  MOD mirror of `FullPathN2V5NoNopComboCCMBorrowCarry`: n2 call×call×max source path
  over `modCode_noNop_v5` (cp + surgical swaps) — composes the MOD call×call 2-combo
  (#7698) + the MOD j=0 max exit iter borrowCarry (#7702); final-post def + simp
  lemmas code-agnostic (reused from DIV `FullPathN2V5NoNopComboCCM`).  Brick 23 of
...
theorem divK_loop_n2_call_call_max_from_source_exact_loopIterScratch_v5_noNop_borrowCarry_modCode
    (sp base : Word)
