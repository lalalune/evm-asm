...
/-- Generalization of `evm_mod_n3_to_loopSetup_spec_within_v4_noNop` allowing
    any initial `x9` value.  The loopSetup instruction overwrites `x9 ← 4 - n`
    before the loop body starts, so the initial value is irrelevant. -/
theorem evm_mod_n3_to_loopSetup_spec_within_v4_noNop_anyX9 (sp base : Word)
    (x9Init : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
