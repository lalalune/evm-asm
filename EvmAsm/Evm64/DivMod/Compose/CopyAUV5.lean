/-
  EvmAsm.Evm64.DivMod.Compose.CopyAUV5

  v5 copy-AU brick (9 steps, copyAUOff → loopSetupOff) over `divCode_noNop_v5`.
  ...
  v5 code surface via the block-6 subsumption `sharedNoNop_v5_b6_div`.  Second
...
/-- v5 copy-AU (shift = 0): copy `a[0..3]` into `u[0..3]`, zero `u[4]`, over
    `divCode_noNop_v5`, from `copyAUOff` to `loopSetupOff`. -/
theorem divK_copyAU_spec_within_v5_noNop (sp base : Word)
    (a0 a1 a2 a3 u0 u1 u2 u3 u4 v5 : Word) :
    cpsTripleWithin 9 (base + copyAUOff) (base + loopSetupOff) (divCode_noNop_v5 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) **
  ...
  exact cpsTripleWithin_extend_code sharedNoNop_v5_b6_div hbody
