/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN1V5ToDenorm

  v5 n=1 entry→denorm composition: chains the full preloop
  (`evm_div_n1_to_loopSetup_spec_v5_noNop`, #7297) onto the loop-at-shape
  theorem (`divK_loop_n1_call_unified_v5_of_shape`, #7284) over the n=1 capped
  v5 loop.  The loop is proven over `sharedDivModCodeNoNop_v5`; it is extended up
  to `divCode_noNop_v5` (via `sharedDivModCodeNoNop_v5_sub_divCode_noNop_v5`) so it
  composes with the preloop.  The glue matches the preloop post `loopSetupPost`
  against the loop pre `loopN1UnifiedPreV5` after unfolding the irreducible
  normalized-shape defs (`fullDivN1NormV`/`fullDivN1NormU`/`fullDivN1Shift`).
...
/-- Full n=1 path from entry to the denormalization epilogue over
    `divCode_noNop_v5` (shift ≠ 0): preloop + capped v5 loop at normalized shape. -/
theorem evm_div_n1_to_denorm_spec_v5_noNop (sp base : Word)
  ...
  -- 1. Preloop: base → base + loopBodyOff
  ...
  have hPreF := cpsTripleWithin_frameR
    ((.x11 ↦ᵣ v11Old) ** ((sp + signExtend12 3976) ↦ₘ jMem) **
     ...
     ((sp + signExtend12 3936) ↦ₘ scratchMem) ** regOwn .x1)
    (by pcFree) hPre
  -- 2. Loop at normalized shape: loopBodyOff → denormOff (extend shared → div code)
  have hLoop0 := divK_loop_n1_call_unified_v5_of_shape sp jMem (1 : Word) shift u0
    (a0 >>> (antiShift.toNat % 64)) v11Old antiShift
    (0 : Word) (0 : Word) (0 : Word) (0 : Word) retMem dMem dloMem scratch_un0 scratchMem base halign
