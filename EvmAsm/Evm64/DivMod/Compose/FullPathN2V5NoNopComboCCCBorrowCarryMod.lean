/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboCCCBorrowCarryMod

  MOD mirror of `FullPathN2V5NoNopComboCCCBorrowCarry`: n2 call×call×call source path
  over `modCode_noNop_v5` (cp + surgical swaps) — composes the MOD call×call 2-combo
  (#7698) + the MOD j=0 call exit iter borrowCarry (#7703); final-post def + simp
  lemmas code-agnostic (reused from DIV `FullPathN2V5NoNopComboCCC`).  Brick 24 of
  the n=2 MOD loop body — completes all 8 three-digit combos.
...
theorem divK_loop_n2_call_call_call_from_source_exact_loopIterScratch_v5_noNop_borrowCarry_modCode
    (sp base : Word)
  ...
  have JCC := divK_loop_n2_call_call_from_source_exact_loopIterScratch_v5_noNop_borrowCarry_modCode
  ...
  have J0 := divK_loop_body_n2_call_j0_exact_loopIterScratch_v5_noNop_borrowCarry_modCode sp base
