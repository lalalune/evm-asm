/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboTMTBorrowCarryMod

  MOD mirror of `FullPathN2V5NoNopComboTMTBorrowCarry`: n2 call×max×call source path
  over `modCode_noNop_v5` (cp + surgical swaps) — composes the MOD call×max 2-combo
  (#7700) + the MOD j=0 call exit iter borrowCarry (#7703); final-post def + simp
  lemmas code-agnostic (reused from DIV `FullPathN2V5NoNopComboTMT`).  Brick 22 of
...
theorem divK_loop_n2_call_max_call_from_source_exact_loopIterScratch_v5_noNop_borrowCarry_modCode
    (sp base : Word)
