/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopCallExactX1

  v5/no-NOP norm-pre wrappers for the n=2 DIV call-path exact-x1 loop bodies
  (call+skip and call+addback, j=0 and j>0), over `divCode_noNop_v5`.  Mirror of
  the exact-x1 call-path norm theorems in `FullPathN2V4NoNop`, lifting the raw v5
  exact-x1 call bodies to `divCode_noNop_v5` via the v5 code subsumption and
  hiding addresses behind the code-agnostic NoX1 NormPre defs reused from the v4
  wrappers.  Takes the v5 borrow/carry2 hypotheses directly.
-/
