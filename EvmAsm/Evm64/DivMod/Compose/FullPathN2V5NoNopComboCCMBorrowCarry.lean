/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboCCMBorrowCarry

  Borrow-dispatched variant of the n=2 call×call×max source path
  (FullPathN2V5NoNopComboCCM): each digit's carry required only conditionally on
  that digit's runtime borrow.  Composes the call×call prefix borrowCarry (#7435)
...
theorem divK_loop_n2_call_call_max_from_source_exact_loopIterScratch_v5_noNop_borrowCarry
    (sp base : Word)
