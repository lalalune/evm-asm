/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopComboTMTBorrowCarry

  Borrow-dispatched variant of the n=2 call_max_call source path (ComboTMT): each digit's
  carry required only conditionally on its runtime borrow.
-/
...
theorem divK_loop_n2_call_max_call_from_source_exact_loopIterScratch_v5_noNop_borrowCarry
    (sp base : Word)
  ...
  have JCM := divK_loop_n2_call_max_from_source_exact_loopIterScratch_v5_noNop_borrowCarry
