/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN2V5NoNopMaxIter

  v5/no-NOP iter-ready max-path loop bodies for n=2 (j=2 and j=1), over
  `divCode_noNop_v5`.  Mirror of `divK_loop_body_n2_max_j{2,1}_exact_loopIterScratch_v4_noNop`
  (FullPathN2V4NoNop): case on the c3 borrow guard, dispatch to the v5 max-path
  norm bodies (addback / skip, #7383), and weaken the post to `loopIterPostN2Max`
  via the (code-agnostic) `loopIterPostN2Max_{addback,skip}` bridges.  These are
  the iteration-ready max bodies the unified n=2 v5 loop's 8-way dispatch consumes
...
/-- Unified j=2 N2 max iteration over v5/no-NOP code, preserving concrete `x1`
    and carrying the v5 scratch cells as frame. -/
theorem divK_loop_body_n2_max_j2_exact_loopIterScratch_v5_noNop (sp base : Word)
  ...
  · have hborrow_nz :
      ...
      rw [if_pos hborrow]
      decide
    have J := divK_loop_body_n2_max_addback_jgt0_beq_norm_v5_noNop
...
/-- Unified j=1 N2 max iteration over v5/no-NOP code, preserving concrete `x1`
    and carrying the v5 scratch cells as frame. -/
theorem divK_loop_body_n2_max_j1_exact_loopIterScratch_v5_noNop (sp base : Word)
    (jOld v5Old v6Old v7Old v10Old v11Old v2Old : Word)
    (v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld raVal : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (hbltu : ¬BitVec.ult u2 v1)
