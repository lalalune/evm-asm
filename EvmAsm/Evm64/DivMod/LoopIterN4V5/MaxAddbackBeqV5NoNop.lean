/-
  EvmAsm.Evm64.DivMod.LoopIterN4V5.MaxAddbackBeqV5NoNop

  v5 n=4 max+addback-beq loop body (j=0) over `sharedDivModCodeNoNop_v5`, with the
  `loopBodyN4MaxSkipJ0Pre` precondition shared with the max-skip body.  Mirror of
  the v5 n=3 max-addback-beq body (`LoopIterN3V5/MaxAddbackBeqV5NoNop`) with the
  n=4 trial window (uHi=uTop, uLo=u3, vTop=v3), the n=4 addresses
  (`u_addr_eq_n4`/`vtop_eq_v3_n4`), and the n=4 pre/post defs
  (`loopBodyN4MaxSkipJ0Pre`/`loopBodyN4AddbackBeqPost`).  The max path skips the
  div128 trial (`qHat = signExtend12 4095`, a constant), then does the single/double
  addback correction (BEQ variant, handling both carry=0 and carry≠0 internally).
  Branch 4/4 of the n=4 v5 loop.  Bead `evm-asm-wbc4i.8`.
-/
...
/-- v5 n=4 max+addback-beq j=0 loop body (loopBodyOff → denormOff). -/
theorem divK_loop_body_n4_max_addback_j0_beq_v5_spec_within_noNop
  ...
  have TF := divK_trial_max_full_v5_spec_within_noNop sp (0 : Word) (4 : Word)
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by rw [loopBodyN4MaxSkipJ0Pre_unfold] at hp; xperm_hyp hp)
    (fun h hp => by
