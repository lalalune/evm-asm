/-
  EvmAsm.Evm64.DivMod.LoopIterN4V5.MaxSkipV5NoNop

  v5 n=4 max+skip loop body (j=0) over `sharedDivModCodeNoNop_v5`.  Direct mirror
  of the v4 n=4 max+skip body (`LoopIterN4MaxV4NoNop`,
  `divK_loop_body_n4_max_skip_j0_v4_spec_within_noNop`) with the three loop-body
  bricks (and the code surface) swapped v4 → v5: `divK_trial_max_full_v5`,
  `divK_mulsub_correction_skip_v5`, `divK_store_loop_j0_v5`.  The max path skips
  the div128 trial (`qHat = signExtend12 4095`, a constant), so the n=4 pre/post
  defs (`loopBodyN4MaxSkipJ0Pre`/`Post`) are version-agnostic and reused as-is;
  the n=4 addresses (`u_addr_eq_n4`/`vtop_eq_v3_n4`) are unchanged.
  Branch 3/4 of the n=4 v5 loop.  Bead `evm-asm-wbc4i.8`.
-/
...
/-- No-NOP/v5 loop body cpsTripleWithin for n=4, max+skip, j=0 (ends at denormOff). -/
theorem divK_loop_body_n4_max_skip_j0_v5_spec_within_noNop
