/-
  EvmAsm.Evm64.DivMod.LoopIterN3V5.MaxSkipV5NoNop

  No-NOP/v5 replays for n=3 max+skip loop-body specs.  Direct mirror of
  `LoopIterN3MaxV4NoNop` with the v5 bricks (divK_trial_max_full_v5,
  divK_mulsub_correction_skip_v5, divK_store_loop_{j0,jgt0}_v5) over
  `sharedDivModCodeNoNop_v5`.  The max path skips the div128 trial (uses
  `signExtend12 4095`), so the proof is the v4 body with the three loop-body
  bricks (and the code surface) swapped v4 → v5; the n=3 addresses
  (`u_addr_eq_n3`/`vtop_eq_v2_n3`) and pre/post defs are unchanged.
...
/-- No-NOP/v5 variant of `divK_loop_body_n3_max_skip_j1_v4_spec_within_noNop`
    (first n=3 outer iteration, j=1, max path). -/
theorem divK_loop_body_n3_max_skip_j1_v5_spec_within_noNop
...
/-- No-NOP/v5 variant of `divK_loop_body_n3_max_skip_j0_v4_spec_within_noNop`
    (last n=3 outer iteration, j=0, max path; ends at `denormOff`). -/
theorem divK_loop_body_n3_max_skip_j0_v5_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (base : Word)
    (hbltu : ¬BitVec.ult u3 v2) :
