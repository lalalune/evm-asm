/-
  EvmAsm.Evm64.DivMod.LoopIterN3V5.CallSkipExactX1V5NoNop

  v5 n=3 call+skip j=0 loop body over `sharedDivModCodeNoNop_v5`, preserving the
  concrete caller `x1` (= `raVal`) through the v5 div128 trial call — the
  exact-x1 variant the n=3 call exact-jN / combo path consumes (x1 is live
  through the div128 call, unlike the max path).  Mirror of the v4 analog
  (`LoopIterN3CallV4NoNop` :257) with the v5 NAMED exact-x1 trial
...
/-- v5 n=3 call+skip j=0 loop-body post (no x1; mirror of
    `loopBodyN3CallSkipJ0PostV4NoX1` with v5 trial defs). -/
@[irreducible]
def loopBodyN3CallSkipJ0PostV5NoX1
...
/-- v5 n=3 call+skip j=0 loop body preserving concrete `x1 = raVal`. -/
theorem divK_loop_body_n3_call_skip_j0_v5_spec_within_noNop_exact_x1
    ...
    cpsTripleWithin 158 (base + loopBodyOff) (base + denormOff) (sharedDivModCodeNoNop_v5 base)
      (loopBodyN3CallSkipJ0PreV4NoX1 sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
        ...
        (.x1 ↦ᵣ raVal))
      (loopBodyN3CallSkipJ0PostV5NoX1 sp base v0 v1 v2 v3 u0 u1 u2 u3 uTop scratchMem **
        (.x1 ↦ᵣ raVal)) := by
  ...
  exact cpsTripleWithin_weaken
    ...
    (fun h hp => by
      unfold loopBodyN3CallSkipJ0PostV5NoX1
...
/-- v5 n=3 call+skip j>0 loop-body post (no x1; mirror of
    `loopBodyN3CallSkipJgt0PostV4NoX1` with v5 trial defs). -/
@[irreducible]
def loopBodyN3CallSkipJgt0PostV5NoX1
...
/-- v5 n=3 call+skip j=1 loop body preserving concrete `x1 = raVal`. -/
theorem divK_loop_body_n3_call_skip_j1_v5_spec_within_noNop_exact_x1
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld raVal : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (base : Word)
    ...
    cpsTripleWithin 158 (base + loopBodyOff) (base + loopBodyOff) (sharedDivModCodeNoNop_v5 base)
      (loopBodyN3CallSkipJgt0PreV4NoX1 sp (1 : Word) jOld v5Old v6Old v7Old v10Old v11Old v2Old
        v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld retMem dMem dloMem scratchUn0 scratchMem **
        (.x1 ↦ᵣ raVal))
      (loopBodyN3CallSkipJgt0PostV5NoX1 sp base (1 : Word) v0 v1 v2 v3 u0 u1 u2 u3 uTop scratchMem **
        (.x1 ↦ᵣ raVal)) := by
  ...
  let qHat := divKTrialCallV5QHat u3 u2 v2
  let scratchOut := divKTrialCallV5ScratchOut u3 u2 v2 scratchMem
  have TF := divK_trial_call_full_v5_named_spec_within_noNop_exact_x1 sp (1 : Word) (3 : Word)
    jOld v5Old v6Old v7Old v10Old v11Old v2Old
    u3 u2 v2 retMem dMem dloMem scratchUn0 scratchMem raVal base
    halign hbltu
  unfold divKTrialCallFullPostV5NamedExactX1 at TF
  ...
  rw [u_addr8_eq_n3] at TF
  rw [vtop_eq_v2_n3] at TF
  have MCS := divK_mulsub_correction_skip_v5_spec_within_noNop sp qHat (1 : Word)
    v0 v1 v2 v3 u0 u1 u2 u3 uTop
    x9Exit q0'' dHi x7Exit q1'' (base + div128CallRetOff) base
    hborrow
  unfold divKMulsubCorrectionSkipPre at MCS
  ...
  let un3 := u3 - fs3; let c3 := pc3 + bs3
  let u4_new := uTop - c3
  have SL := divK_store_loop_jgt0_v5_spec_within_noNop_exact_x1 sp (1 : Word) qHat u4_new
    (0 : Word) qOld raVal base EvmAsm.Evm64.DivMod.AddrNorm.slt_jpos_1
  intro_lets at SL
  have TFf := cpsTripleWithin_frameR
    (((sp + signExtend12 32) ↦ₘ v0) ** ((uBase + signExtend12 0) ↦ₘ u0) **
     ((sp + signExtend12 40) ↦ₘ v1) ** ((uBase + signExtend12 4088) ↦ₘ u1) **
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by xperm_hyp hp)
    (fun h hp => by
      unfold loopBodyN3CallSkipJgt0PostV5NoX1
      unfold loopBodyN3SkipPost loopBodySkipPost loopExitPost
      unfold mulsubN4
      dsimp only []
      rw [sepConj_assoc'] at hp
