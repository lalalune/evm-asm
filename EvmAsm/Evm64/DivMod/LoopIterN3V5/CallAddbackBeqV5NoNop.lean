...
/-- v5 n=3 call+addback-beq j>0 loop-body post (j-parameterized; mirror of
    `loopBodyN3CallAddbackJgt0PostV4` with v5 trial defs). -/
@[irreducible]
def loopBodyN3CallAddbackBeqJgt0PostV5
...
/-- v5 n=3 call+addback-beq j>0 loop body (loopBodyOff → loopBodyOff loop-back),
    with the `loopBodyN3CallAddbackJgt0PreV4` precondition. -/
theorem divK_loop_body_n3_call_addback_jgt0_beq_v5_spec_within_noNop (j : Word)
  ...
  have TF := divK_trial_call_full_v5_named_spec_within_noNop sp j (3 : Word)
    jOld v5Old v6Old v7Old v10Old v11Old v2Old
    u3 u2 v2 retMem dMem dloMem scratchUn0 scratchMem base
    halign hbltu
  unfold divKTrialCallFullPostV5Named at TF
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by
      ...
      xperm_hyp hp)
    (fun h hp => by
      unfold loopBodyN3CallAddbackBeqJgt0PostV5
      unfold loopBodyN3AddbackBeqPost loopBodyAddbackBeqPost loopExitPost
      rw [sepConj_assoc'] at hp
      xperm_hyp hp)
    full
