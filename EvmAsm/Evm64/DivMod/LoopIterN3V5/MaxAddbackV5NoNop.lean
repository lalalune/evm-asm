/-
  EvmAsm.Evm64.DivMod.LoopIterN3V5.MaxAddbackV5NoNop

  No-NOP/v5 replays for n=3 max+addback loop-body specs (the max path when the
  mulsub borrows, requiring an addback correction).  Direct mirror of
  `LoopIterN3MaxAddbackV4NoNop` with the v5 bricks (divK_trial_max_full_v5,
  divK_mulsub_correction_addback_beq_v5, divK_store_loop_{j0,jgt0}_v5) over
  `sharedDivModCodeNoNop_v5`.  The max path skips the div128 trial (uses
  `signExtend12 4095`), so this is the v4 body with the code surface + three
  loop-body bricks swapped v4 → v5; the n=3 addresses and pre/post are unchanged.
...
/-- No-NOP/v5 loop body cpsTripleWithin for n=3, max+addback, j=0. -/
theorem divK_loop_body_n3_max_addback_beq_j0_v5_spec_within_noNop
    ...
    cpsTripleWithin 152 (base + loopBodyOff) (base + denormOff) (sharedDivModCodeNoNop_v5 base)
  ...
  have TF := divK_trial_max_full_v5_spec_within_noNop sp (0 : Word) (3 : Word)
...
/-- No-NOP/v5 loop body cpsTripleWithin for n=3, max+addback, j=1. -/
theorem divK_loop_body_n3_max_addback_beq_j1_v5_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (base : Word)
    (hbltu : ¬BitVec.ult u3 v2)
    ...
    (if BitVec.ult uTop (mulsubN4_c3 (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3) then (1 : Word) else 0) ≠ (0 : Word) →
    cpsTripleWithin 152 (base + loopBodyOff) (base + loopBodyOff) (sharedDivModCodeNoNop_v5 base)
  ...
  let vtopBase := sp + ((3 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_max_full_v5_spec_within_noNop sp (1 : Word) (3 : Word)
    jOld v5Old v6Old v7Old v10Old v11Old u3 u2 v2 base hbltu
  dsimp only [] at TF
  rw [u_addr_eq_n3] at TF
  rw [u_addr8_eq_n3] at TF
  ...
  intro_lets at MCA
  have MCA0 := MCA hcarry2_nz hborrow
  have SL := divK_store_loop_jgt0_v5_spec_within_noNop sp (1 : Word) q_out u4_out carryOut qOld base slt_jpos_1
  intro_lets at SL
  have TFf := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ v2Old) **
     ((sp + signExtend12 32) ↦ₘ v0) ** ((uBase + signExtend12 0) ↦ₘ u0) **
