...
/-- N1 callable wrapper over the bundled selected-if-borrow shape evidence.

    This is the narrow callable boundary the public N1 stack wrapper can target:
    branch/path/carry and semantic arithmetic facts are carried by the private
    evidence bundle instead of separate callback or semantic-evidence premises. -/
theorem evm_div_callable_v4_n1_stack_pre_to_callable_post_scratch_shape_callableEvidence_inputHdivRoute
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
...
/-- N1 callable wrapper deriving all-true path evidence from the public
    all-phases overestimate/no-wrap surface before building the private
    callable evidence bundle.

    This keeps `N1AllTruePathEvidence` out of the callable boundary; selected
    branch facts remain explicit because the all-true witnesses do not imply the
    selected call/max/max/max no-ult branch facts. -/
theorem evm_div_callable_v4_n1_stack_pre_to_callable_post_scratch_shape_branchFactsAllPhasesSemanticFacts_inputHdivRoute
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
...
/-- N1 callable wrapper deriving selected branch facts from the named
    selected-if-borrow input package, and all-true path evidence from the public
    all-phases overestimate/no-wrap surface.

    This keeps `N1CallableSelectedIfBorrowBranchFacts` and raw branch booleans
    out of the callable boundary; the selected input package is still an
    intermediate surface for the final public wrapper to discharge. -/
theorem evm_div_callable_v4_n1_stack_pre_to_callable_post_scratch_shape_selectedInputAllPhasesSemanticFacts_inputHdivRoute
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divStackDispatchPostCallableExactFrame sp a b raVal
        (signExtend12 4095 : Word) **
       memOwn (sp + signExtend12 3936)) := by
  exact evm_div_callable_v4_n1_stack_pre_to_callable_post_scratch_shape_branchFactsAllPhasesSemanticFacts_inputHdivRoute
    sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
    ...
    (FullDivN1CallMaxmaxmaxSelectedIfBorrowSemanticEvidenceV4_branchFacts
      ...
      retMem dMem dloMem scratchUn0 scratchMem raVal
      ⟨hselected, hfacts⟩)
    hr3_lt hr2_lt hr1_lt hr2_inv hr1_inv hfinal_inv hpath hfacts
