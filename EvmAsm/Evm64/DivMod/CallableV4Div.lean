...
/-- Named-post wrapper for v4 callable DIV body proofs that preserve exact
    caller-framed `x1`/`x9`. -/
theorem evm_div_callable_v4_spec_from_noNop_exact_frame
    (sp base x9Val raVal : Word) (a b : EvmWord)
    (v2 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 : Word)
  ...
  exact evm_div_callable_v4_spec_from_noNop_preserving_x1_x9
...
/-- Trial-bundled N2 DIV v4 callable wrapper with existential scratch output.

    This removes the explicit branch booleans from the callable surface: the
    mechanical trial witness bundle owns them, while callers still provide the
    remaining carry and arithmetic obligations needed to assemble the V4 path
    predicate. -/
theorem evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_trial
  ...
  exact evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_path
    bltu_2 bltu_1 bltu_0 sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
...
/-- Auto-trial N2 DIV v4 callable wrapper with existential scratch output.

    This constructs the mechanical V4 trial witness bundle internally, leaving
    only carry and arithmetic path obligations on the callable surface. -/
theorem evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_autoTrial
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divStackDispatchPostCallableExactFrame sp a b raVal
        (signExtend12 4095 : Word) **
       memOwn (sp + signExtend12 3936)) := by
  exact evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_trial
    sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
...
/-- Limb-or nonzero N2 DIV v4 callable wrapper with auto-trial witnesses. -/
theorem evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_limbNz
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divStackDispatchPostCallableExactFrame sp a b raVal
        (signExtend12 4095 : Word) **
       memOwn (sp + signExtend12 3936)) := by
  exact evm_div_callable_v4_n2_stack_pre_to_callable_post_scratch_autoTrial
    sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
...
/-- Path-bundled N3 DIV v4 callable wrapper with existential scratch output.

    This lifts the no-NOP N3 v4 path wrapper through the callable `ret` shim and
    weakens the trial-call scratch cell to `memOwn`, matching the dispatcher
    consumer surface. -/
theorem evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_path
...
/-- Trial-bundled N3 DIV v4 callable wrapper with existential scratch output.

    This removes the explicit branch booleans from the callable surface: the
    mechanical trial witness bundle owns them, while callers still provide the
    remaining carry and arithmetic obligations needed to assemble the V4 path
    predicate. -/
theorem evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_trial
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
  ...
  exact evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_path
    bltu_1 bltu_0 sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
...
/-- Auto-trial N3 DIV v4 callable wrapper with existential scratch output.

    This constructs the mechanical V4 trial witness bundle internally, leaving
    only carry and arithmetic path obligations on the callable surface. -/
theorem evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_autoTrial
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divStackDispatchPostCallableExactFrame sp a b raVal
        (signExtend12 4095 : Word) **
       memOwn (sp + signExtend12 3936)) := by
  exact evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_trial
    sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
...
/-- Limb-or nonzero N3 DIV v4 callable wrapper with auto-trial witnesses. -/
theorem evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_limbNz
    (sp base : Word) (a b : EvmWord)
    (v5 v6 v7 v10 v11Old : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem : Word)
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divStackDispatchPostCallableExactFrame sp a b raVal
        (signExtend12 4095 : Word) **
       memOwn (sp + signExtend12 3936)) := by
  exact evm_div_callable_v4_n3_stack_pre_to_callable_post_scratch_autoTrial
    sp base a b
    v5 v6 v7 v10 v11Old
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7
    nMem shiftMem jMem retMem dMem dloMem scratchUn0 scratchMem raVal
