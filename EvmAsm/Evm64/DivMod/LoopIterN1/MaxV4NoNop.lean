/-
  EvmAsm.Evm64.DivMod.LoopIterN1.MaxV4NoNop

  No-NOP/v4 replays for n=1 BLTU-not-taken max+skip loop-body specs.
-/
...
/-- No-NOP/v4 loop body cpsTripleWithin for n=1, max+skip, j=0.
    Since j=0, the BGE loop-back is not taken, giving a cpsTripleWithin to denorm. -/
theorem divK_loop_body_n1_max_skip_j0_v4_spec_within_noNop
...
/-- No-NOP/v4 loop body cpsTripleWithin for n=1, max+skip, j>0.
    Since j>0, the BGE loop-back is taken, giving a cpsTripleWithin back to loopBody. -/
theorem divK_loop_body_n1_max_skip_jgt0_v4_spec_within_noNop (j : Word)
