#!/usr/bin/env bash
# codegen-zisk-chain-compute-max-blob-count-check.sh -- PR-K285.
set -euo pipefail
...
echo "==> emit zisk_chain_compute_max_blob_count ELF"
lake exe codegen --program zisk_chain_compute_max_blob_count --halt linux93 \
  -o gen-out/zisk_chain_compute_max_blob_count

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" blob_counts="$2" exp_status="$3" exp_max="$4"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_compute_max_blob_count_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_compute_max_blob_count_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_compute_max_blob_count.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_compute_max_blob_count_${name}.emu.log" 2>&1 || true
...
run_case "single_1"     "[1]"                 0 1 || FAILED=1
run_case "single_6"     "[6]"                 0 6 || FAILED=1
run_case "three_inc"    "[1, 2, 3]"           0 3 || FAILED=1
run_case "three_mixed"  "[3, 1, 2]"           0 3 || FAILED=1
run_case "five_peak"    "[0, 2, 6, 1, 4]"     0 6 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_compute_max_blob_count finds max blob count"
  exit 0
else
  echo "==> FAIL"
  exit 1
