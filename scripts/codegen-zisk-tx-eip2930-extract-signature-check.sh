#!/usr/bin/env bash
# codegen-zisk-tx-eip2930-extract-signature-check.sh -- PR-K140.
#
# Extract (y_parity, r, s) from the inner RLP of an EIP-2930
...
echo "==> emit zisk_tx_eip2930_extract_signature ELF"
lake exe codegen --program zisk_tx_eip2930_extract_signature --halt linux93 \
  -o gen-out/zisk_tx_eip2930_extract_signature
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_tx_eip2930_extract_signature_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_tx_eip2930_extract_signature_${name}.output"
...
ALICE = bytes([0xaa] * 20)
inner = [1, 42, 10**9, 21000, ALICE, 10**18, b'', [], y, r, s]
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_tx_eip2930_extract_signature.elf \
    -i "$in_file" -o "$out_file" -n 1000000 \
    >"$REPO_ROOT/gen-out/zisk_tx_eip2930_extract_signature_${name}.emu.log" 2>&1 || true
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: tx_eip2930_extract_signature recovers (y_parity, r, s) correctly"
  exit 0
else
  echo "==> FAIL"
  exit 1
