#!/usr/bin/env bash
# codegen-zisk-chain-compute-total-gas-limit-check.sh -- PR-K264.
set -euo pipefail
...
echo "==> emit zisk_chain_compute_total_gas_limit ELF"
lake exe codegen --program zisk_chain_compute_total_gas_limit --halt linux93 \
  -o gen-out/zisk_chain_compute_total_gas_limit

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" gas_limits="$2" exp_status="$3" exp_total="$4"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_compute_total_gas_limit_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_compute_total_gas_limit_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_compute_total_gas_limit.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_compute_total_gas_limit_${name}.emu.log" 2>&1 || true
...
run_case "three_same"   "[30000000,30000000,30000000]"    0 90000000  || FAILED=1
run_case "three_mixed"  "[10000000,20000000,30000000]"    0 60000000  || FAILED=1
run_case "five_small"   "[100,200,300,400,500]"           0 1500      || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_compute_total_gas_limit sums gas_limit across N headers"
  exit 0
else
  echo "==> FAIL"
  exit 1
