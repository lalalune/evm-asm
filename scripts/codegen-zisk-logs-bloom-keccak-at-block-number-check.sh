#!/usr/bin/env bash
# codegen-zisk-logs-bloom-keccak-at-block-number-check.sh
#
# Number-keyed keccak256(logs_bloom) extractor.
#
# Output (40 bytes):
...
echo "==> emit zisk_logs_bloom_keccak_at_block_number ELF"
lake exe codegen --program zisk_logs_bloom_keccak_at_block_number \
  --halt linux93 \
  -o gen-out/zisk_logs_bloom_keccak_at_block_number
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_lbkbn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_lbkbn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_lbkbn_${name}.expected"
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_logs_bloom_keccak_at_block_number.elf \
    -i "$in_file" -o "$out_file" -n 4000000 \
    >"$REPO_ROOT/gen-out/zisk_lbkbn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "all_zero_bloom_at_101"   all_zero_bloom 101 || FAILED=1
run_case "patterned_bloom"         patterned_bloom 101 || FAILED=1
run_case "pick_second_of_two"      pick_second_of_two 101 || FAILED=1
run_case "number_not_in_section"   number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: logs_bloom_keccak_at_block_number end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
