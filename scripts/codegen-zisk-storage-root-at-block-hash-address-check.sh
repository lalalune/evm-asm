#!/usr/bin/env bash
# codegen-zisk-storage-root-at-block-hash-address-check.sh
#
# Hash-keyed historical storage_root extractor. Find header
# by block_hash, walk to account, return storage_root field.
#
# Output (40 bytes):
#   bytes  0.. 8 : status (0..6)
#   bytes  8..40 : storage_root (32 B; EMPTY_TRIE_ROOT on absent)
set -euo pipefail
...
echo "==> emit zisk_storage_root_at_block_hash_address ELF"
lake exe codegen --program zisk_storage_root_at_block_hash_address \
  --halt linux93 \
  -o gen-out/zisk_storage_root_at_block_hash_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_srbh_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_srbh_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_srbh_${name}.expected"
...
if mode == 'present_eoa_empty_storage':
...
elif mode == 'present_contract_populated':
    # Contract with populated storage_root.
    ...
    block_hash = k256(h0)
    addr = ALICE
    expected = struct.pack('<Q', 0) + sr_storage
    ...
    expected = struct.pack('<Q', 4) + EMPTY_TRIE
else:
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_storage_root_at_block_hash_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_srbh_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "present_eoa_empty_storage"      present_eoa_empty_storage || FAILED=1
run_case "present_contract_populated"     present_contract_populated || FAILED=1
run_case "block_hash_miss"                block_hash_miss || FAILED=1
run_case "absent_account_default_empty"   account_absent || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: storage_root_at_block_hash_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
