#!/usr/bin/env bash
# codegen-zisk-nonce-at-block-hash-address-check.sh
#
# Hash-keyed historical nonce extractor. Completes the
# hash-keyed per-field family.
#
# Output (16 bytes):
...
echo "==> emit zisk_nonce_at_block_hash_address ELF"
lake exe codegen --program zisk_nonce_at_block_hash_address \
  --halt linux93 \
  -o gen-out/zisk_nonce_at_block_hash_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_nbh_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_nbh_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_nbh_${name}.expected"
...
if mode == 'fresh_nonce_zero':
...
elif mode == 'small_nonce':
    n = 5
    acct = encode_account(n, 0, EMPTY_TRIE, EMPTY_CODE_HASH)
    sr, witness_state = state_trie_one(ALICE, acct)
    h0 = encode_header(sr)
    witness_headers = build_ssz_section([h0])
    block_hash = k256(h0); addr = ALICE
    ...
    acct = encode_account(5, 0, EMPTY_TRIE, EMPTY_CODE_HASH)
    sr, witness_state = state_trie_one(ALICE, acct)
    h0 = encode_header(sr)
    witness_headers = build_ssz_section([h0])
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_nonce_at_block_hash_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_nbh_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "fresh_eoa_nonce_zero"          fresh_nonce_zero || FAILED=1
run_case "small_nonce_5"                 small_nonce || FAILED=1
run_case "max_u64_nonce"                 max_u64 || FAILED=1
run_case "absent_account_default_zero"   absent || FAILED=1
run_case "block_hash_miss"               block_hash_miss || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: nonce_at_block_hash_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
