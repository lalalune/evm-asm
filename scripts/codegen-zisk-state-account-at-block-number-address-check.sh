#!/usr/bin/env bash
# codegen-zisk-state-account-at-block-number-address-check.sh
#
# Number-keyed historical account walk.
#
# Output (112 bytes):
...
echo "==> emit zisk_state_account_at_block_number_address ELF"
lake exe codegen --program zisk_state_account_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_state_account_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_sabn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_sabn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_sabn_${name}.expected"
...
if mode == 'present':
    ...
    h0 = encode_header(100, b'\\xee'*32)  # decoy header
    h1 = encode_header(101, sr)            # match target=101
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_state_account_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_sabn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "present_at_target_number"    present 101 || FAILED=1
run_case "absent_account_at_target"    absent_account 101 || FAILED=1
run_case "number_not_in_section"       number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: state_account_at_block_number_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
