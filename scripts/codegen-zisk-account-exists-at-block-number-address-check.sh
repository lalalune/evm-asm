#!/usr/bin/env bash
# codegen-zisk-account-exists-at-block-number-address-check.sh
#
# Number-keyed account presence predicate. Boolean mirror of
# account_exists_at_block_hash_address (#7456).
#
# Output (16 bytes):
...
echo "==> emit zisk_account_exists_at_block_number_address ELF"
lake exe codegen --program zisk_account_exists_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_account_exists_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_aebn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_aebn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_aebn_${name}.expected"
...
if mode == 'eoa_present':
    # Even a fully-empty EOA that's in the trie -> predicate 1.
    ...
    acct = encode_account(42, 1000, EMPTY_TRIE, k256(bytes.fromhex('6000')))
    sr, witness_state = state_trie_one(ALICE, acct)
    ...
    # ALICE lookup, only BOB in trie -> predicate 0.
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_account_exists_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_aebn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "eoa_present_at_target_number"   eoa_present 101 || FAILED=1
run_case "contract_present"               contract_present 101 || FAILED=1
run_case "absent_address_pred_zero"       absent_address 101 || FAILED=1
run_case "number_not_in_section"          number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: account_exists_at_block_number_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
