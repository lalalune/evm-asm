#!/usr/bin/env bash
# codegen-zisk-runtime-extcodehash-check.sh
#
# Validate runtime_dispatcher EXTCODEHASH against the optional
...
RUN_DIR="${RUN_DIR:-gen-out/runtime_extcodehash}"
case "$RUN_DIR" in
  /*) ;;
  *) RUN_DIR="$PWD/$RUN_DIR" ;;
esac
...
if [[ "$FAILED" -ne 0 ]]; then
  echo "==> FAIL: runtime EXTCODEHASH witness" >&2
  exit 1
fi

echo "==> PASS: runtime EXTCODEHASH witness"
