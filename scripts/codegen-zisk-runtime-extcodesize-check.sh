...
make_case long_code present "$ALICE" "$ALICE" 7 1000000000000000000 "00112233445566778899aabbccddeeff10203040"
make_case empty empty "$ALICE" "$ALICE" 0 0 ""
make_case missing missing "$BOB" "$ALICE" 0 0 "6000"
make_case no_context no_context "$ALICE" "$ALICE" 0 0 "6000"

for name in present long_code empty missing no_context; do
  echo "==> pack $name"
