#!/usr/bin/env bash
# codegen-zisk-code-at-block-number-address-check.sh
#
# Number-keyed historical bytecode extractor.
#
# Output (24 bytes):
...
echo "==> emit zisk_code_at_block_number_address ELF"
lake exe codegen --program zisk_code_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_code_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_cabn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_cabn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_cabn_${name}.expected"
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_code_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_cabn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "contract_present_at_target"   contract_present 101 || FAILED=1
run_case "eoa_at_target"                eoa_at_target 101 || FAILED=1
run_case "number_not_in_section"        number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: code_at_block_number_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
