#!/usr/bin/env bash
# codegen-zisk-chain-compute-min-excess-blob-gas-check.sh -- PR-K273.
set -euo pipefail
...
echo "==> emit zisk_chain_compute_min_excess_blob_gas ELF"
lake exe codegen --program zisk_chain_compute_min_excess_blob_gas --halt linux93 \
  -o gen-out/zisk_chain_compute_min_excess_blob_gas

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" vals="$2" exp_status="$3" exp_min="$4"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_compute_min_excess_blob_gas_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_compute_min_excess_blob_gas_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_compute_min_excess_blob_gas.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_compute_min_excess_blob_gas_${name}.emu.log" 2>&1 || true
...
run_case "three_inc"    "[100, 200, 300]"          0 100     || FAILED=1
run_case "three_mixed"  "[300, 100, 200]"          0 100     || FAILED=1
run_case "five_small"   "[500, 100, 400, 200, 300]" 0 100    || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_compute_min_excess_blob_gas finds min excess_blob_gas across N"
  exit 0
else
  echo "==> FAIL"
  exit 1
