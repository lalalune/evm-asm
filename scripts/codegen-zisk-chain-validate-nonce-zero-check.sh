#!/usr/bin/env bash
# codegen-zisk-chain-validate-nonce-zero-check.sh -- PR-K288.
set -euo pipefail
...
echo "==> emit zisk_chain_validate_nonce_zero ELF"
lake exe codegen --program zisk_chain_validate_nonce_zero --halt linux93 \
  -o gen-out/zisk_chain_validate_nonce_zero

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" nonces="$2" exp_status="$3" exp_valid="$4" exp_bad_index="$5"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_validate_nonce_zero_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_validate_nonce_zero_${name}.output"

  uv run --directory execution-specs --quiet python3 -c "
import struct, sys, rlp
def make_header(nonce_int):
    # nonce is field 14, encoded as 8-byte big-endian bytes.
    if nonce_int == 0:
        nonce = b'\\x00' * 8
    else:
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_validate_nonce_zero.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_validate_nonce_zero_${name}.emu.log" 2>&1 || true
...
run_case "five_all_zero"      "[0, 0, 0, 0, 0]"       0 1 0 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_validate_nonce_zero enforces post-merge invariant"
  exit 0
else
  echo "==> FAIL"
  exit 1
