#!/usr/bin/env bash
# codegen-zisk-account-is-empty-at-block-number-address-check.sh
#
...
echo "==> emit zisk_account_is_empty_at_block_number_address ELF"
lake exe codegen --program zisk_account_is_empty_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_account_is_empty_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_aiebn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_aiebn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_aiebn_${name}.expected"
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_account_is_empty_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_aiebn_${name}.emu.log" 2>&1 || true
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: account_is_empty_at_block_number_address (EIP-161) end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
