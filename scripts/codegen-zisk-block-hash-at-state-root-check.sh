#!/usr/bin/env bash
# codegen-zisk-block-hash-at-state-root-check.sh
#
# Reverse lookup: state_root -> block_hash.
#
# Output (40 bytes):
#   bytes  0.. 8 : status (0..2)
#   bytes  8..40 : block_hash (32 B; zero if no match)
set -euo pipefail
...
echo "==> emit zisk_block_hash_at_state_root ELF"
lake exe codegen --program zisk_block_hash_at_state_root \
  --halt linux93 \
  -o gen-out/zisk_block_hash_at_state_root
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_bhsr_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_bhsr_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_bhsr_${name}.expected"
...
if mode == 'three_chain_first':
    ...
    target_sr = seq[0][1]
    matched_hash = k256(seq[0][0])
    expected = struct.pack('<Q', 0) + matched_hash
elif mode == 'three_chain_middle':
    seq = [encode_header(100, 0x44), encode_header(101, 0x55), encode_header(102, 0x66)]
    witness_headers = build_ssz_section([h for h, _ in seq])
    target_sr = seq[1][1]
    matched_hash = k256(seq[1][0])
    expected = struct.pack('<Q', 0) + matched_hash
elif mode == 'three_chain_last':
    seq = [encode_header(100, 0x44), encode_header(101, 0x55), encode_header(102, 0x66)]
    witness_headers = build_ssz_section([h for h, _ in seq])
    target_sr = seq[2][1]
    matched_hash = k256(seq[2][0])
    expected = struct.pack('<Q', 0) + matched_hash
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_block_hash_at_state_root.elf \
    -i "$in_file" -o "$out_file" -n 4000000 \
    >"$REPO_ROOT/gen-out/zisk_bhsr_${name}.emu.log" 2>&1 || true
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: block_hash_at_state_root end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
