#!/usr/bin/env bash
# codegen-zisk-code-hash-at-block-hash-address-check.sh
#
# Hash-keyed historical code_hash extractor.
#
# Output (40 bytes):
#   bytes  0.. 8 : status (0..6)
#   bytes  8..40 : code_hash (32 B; EMPTY_CODE_HASH on absent)
set -euo pipefail
...
echo "==> emit zisk_code_hash_at_block_hash_address ELF"
lake exe codegen --program zisk_code_hash_at_block_hash_address \
  --halt linux93 \
  -o gen-out/zisk_code_hash_at_block_hash_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_chbh_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chbh_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_chbh_${name}.expected"
...
if mode == 'eoa_present':
    ...
    witness_headers = build_ssz_section([h0])
    block_hash = k256(h0); addr = ALICE
    expected = struct.pack('<Q', 4) + EMPTY_CODE_HASH
else:
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_code_hash_at_block_hash_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_chbh_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "eoa_present_empty_code_default"  eoa_present || FAILED=1
run_case "contract_present"                contract_present || FAILED=1
run_case "block_hash_miss"                 block_hash_miss || FAILED=1
run_case "absent_account_default_empty"    absent_account || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: code_hash_at_block_hash_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
