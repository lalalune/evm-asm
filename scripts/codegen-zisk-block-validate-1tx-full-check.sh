#!/usr/bin/env bash
# codegen-zisk-block-validate-1tx-full-check.sh -- PR-K189.
#
# N=1 analogue of K176: header pair + 1-tx tx_root in one call.
set -euo pipefail
...
echo "==> emit zisk_block_validate_1tx_full ELF"
lake exe codegen --program zisk_block_validate_1tx_full --halt linux93 \
  -o gen-out/zisk_block_validate_1tx_full
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_block_validate_1tx_full_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_block_validate_1tx_full_${name}.output"
...
field4 = b'\\xff'*32 if break_tx_root else correct_tx_root
child_num = 102 if break_pair else 101
...
with open(sys.argv[1], 'wb') as f:
    record = struct.pack('<Q', len(parent_rlp)) + \
             ...
             parent_rlp + child_rlp + tx0
    f.write(record)
    pad = (-len(record)) % 8
    if pad: f.write(b'\x00' * pad)
" "$in_file"

  "$ZISKEMU" -e gen-out/zisk_block_validate_1tx_full.elf \
    -i "$in_file" -o "$out_file" -n 10000000 \
    >"$REPO_ROOT/gen-out/zisk_block_validate_1tx_full_${name}.emu.log" 2>&1 || true
...
run_case "fail_pair"          1 0 0 0 || FAILED=1
run_case "fail_tx_root"       0 1 0 0 || FAILED=1
run_case "fail_both"          1 1 0 0 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: block_validate_1tx_full enforces pair + tx_root invariants"
  exit 0
else
  echo "==> FAIL"
  exit 1
