#!/usr/bin/env bash
# codegen-zisk-balance-at-block-number-address-check.sh
#
...
echo "==> emit zisk_balance_at_block_number_address ELF"
lake exe codegen --program zisk_balance_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_balance_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_bbn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_bbn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_bbn_${name}.expected"
...
if mode == 'small_balance':
    ...
    addr = ALICE
    expected = struct.pack('<Q', 0) + be32(bal)
    ...
    addr = ALICE
    expected = struct.pack('<Q', 0) + be32(bal)
elif mode == 'zero_balance_present':
    acct = encode_account(7, 0, EMPTY_TRIE, EMPTY_CODE_HASH)  # nonce=7 makes account distinct
    sr, witness_state = state_trie_one(ALICE, acct)
    h0 = encode_header(101, sr)
    witness_headers = build_ssz_section([h0])
    addr = ALICE
    ...
    witness_headers = build_ssz_section([h0])
    addr = ALICE
    expected = struct.pack('<Q', 4) + be32(0)
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_balance_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_bbn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "small_balance_1eth_at_101"      small_balance 101 || FAILED=1
run_case "big_balance_2_to_200"           big_balance 101 || FAILED=1
run_case "zero_balance_with_nonce"        zero_balance_present 101 || FAILED=1
run_case "absent_account_default_zero"    absent_account 101 || FAILED=1
run_case "number_not_in_section"          number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: balance_at_block_number_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
