#!/usr/bin/env bash
# codegen-zisk-chain-validate-excess-blob-gas-non-decreasing-check.sh -- PR-K274.
set -euo pipefail
...
echo "==> emit zisk_chain_validate_excess_blob_gas_non_decreasing ELF"
lake exe codegen --program zisk_chain_validate_excess_blob_gas_non_decreasing --halt linux93 \
  -o gen-out/zisk_chain_validate_excess_blob_gas_non_decreasing
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_chain_validate_excess_blob_gas_non_decreasing_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_validate_excess_blob_gas_non_decreasing_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_validate_excess_blob_gas_non_decreasing.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_validate_excess_blob_gas_non_decreasing_${name}.emu.log" 2>&1 || true
...
run_case "two_inc"                "[100, 200]"                      0 1 0 || FAILED=1
run_case "two_dec"                "[200, 100]"                      0 0 1 || FAILED=1
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_validate_excess_blob_gas_non_decreasing predicate matches"
  exit 0
else
  echo "==> FAIL"
  exit 1
