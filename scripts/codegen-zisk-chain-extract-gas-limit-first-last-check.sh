#!/usr/bin/env bash
# codegen-zisk-chain-extract-gas-limit-first-last-check.sh -- PR-K265.
set -euo pipefail
...
echo "==> emit zisk_chain_extract_gas_limit_first_last ELF"
lake exe codegen --program zisk_chain_extract_gas_limit_first_last --halt linux93 \
  -o gen-out/zisk_chain_extract_gas_limit_first_last

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" gas_limits="$2" exp_status="$3" exp_first="$4" exp_last="$5"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_extract_gas_limit_first_last_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_extract_gas_limit_first_last_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_extract_gas_limit_first_last.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_extract_gas_limit_first_last_${name}.emu.log" 2>&1 || true
...
run_case "single"         "[30000000]"                          0 30000000 30000000    || FAILED=1
run_case "three_inc"      "[10000000, 20000000, 30000000]"      0 10000000 30000000    || FAILED=1
run_case "three_dec"      "[30000000, 20000000, 10000000]"      0 30000000 10000000    || FAILED=1
run_case "zero_first"     "[0, 100, 200]"                       0 0 200                || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_extract_gas_limit_first_last returns (first, last) gas_limit across N"
  exit 0
else
  echo "==> FAIL"
  exit 1
