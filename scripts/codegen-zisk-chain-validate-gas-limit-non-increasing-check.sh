#!/usr/bin/env bash
# codegen-zisk-chain-validate-gas-limit-non-increasing-check.sh -- PR-K270.
set -euo pipefail
...
echo "==> emit zisk_chain_validate_gas_limit_non_increasing ELF"
lake exe codegen --program zisk_chain_validate_gas_limit_non_increasing --halt linux93 \
  -o gen-out/zisk_chain_validate_gas_limit_non_increasing
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_chain_validate_gas_limit_non_increasing_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_validate_gas_limit_non_increasing_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_validate_gas_limit_non_increasing.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_validate_gas_limit_non_increasing_${name}.emu.log" 2>&1 || true
...
run_case "two_dec"                "[30000000, 20000000]"            0 1 0 || FAILED=1
run_case "two_inc"                "[20000000, 30000000]"            0 0 1 || FAILED=1
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_validate_gas_limit_non_increasing predicate matches"
  exit 0
else
  echo "==> FAIL"
  exit 1
