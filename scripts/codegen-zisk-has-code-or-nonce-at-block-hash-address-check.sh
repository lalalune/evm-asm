#!/usr/bin/env bash
# codegen-zisk-has-code-or-nonce-at-block-hash-address-check.sh
#
...
#
# Spec-distinguishing row matrix (vs account_exists / account_is_empty):
#
...
echo "==> emit zisk_has_code_or_nonce_at_block_hash_address ELF"
lake exe codegen --program zisk_has_code_or_nonce_at_block_hash_address \
  --halt linux93 \
  -o gen-out/zisk_has_code_or_nonce_at_block_hash_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_hcobh_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_hcobh_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_hcobh_${name}.expected"
...
if mode == 'account':
    ...
    # EIP-684 collision iff nonce!=0 OR code_hash!=EMPTY.
    pred = 1 if (nonce != 0 or code_hash != EMPTY_CODE_HASH) else 0
    expected = struct.pack('<Q', 0) + struct.pack('<Q', pred)
elif mode == 'missing':
    # Lookup ALICE, only BOB is in trie -> account absent -> no collision.
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_has_code_or_nonce_at_block_hash_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_hcobh_${name}.emu.log" 2>&1 || true
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: has_code_or_nonce_at_block_hash_address (EIP-684) end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
