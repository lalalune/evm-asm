#!/usr/bin/env bash
# codegen-zisk-chain-compute-min-timestamp-gap-check.sh -- PR-K280.
set -euo pipefail
...
echo "==> emit zisk_chain_compute_min_timestamp_gap ELF"
lake exe codegen --program zisk_chain_compute_min_timestamp_gap --halt linux93 \
  -o gen-out/zisk_chain_compute_min_timestamp_gap
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_chain_compute_min_timestamp_gap_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_compute_min_timestamp_gap_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_compute_min_timestamp_gap.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_compute_min_timestamp_gap_${name}.emu.log" 2>&1 || true
...
run_case "three_varying"        "[100, 112, 150]"               0 12  0 || FAILED=1
run_case "five_min_at_3"        "[100, 200, 300, 305, 405]"     0 5   0 || FAILED=1
run_case "two_dec_fail"         "[200, 100]"                    3 0   1 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_compute_min_timestamp_gap finds shortest gap"
  exit 0
else
  echo "==> FAIL"
  exit 1
