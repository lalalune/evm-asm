#!/usr/bin/env bash
# codegen-zisk-chain-compute-total-basefee-check.sh -- PR-K249.
set -euo pipefail
...
echo "==> emit zisk_chain_compute_total_basefee ELF"
lake exe codegen --program zisk_chain_compute_total_basefee --halt linux93 \
  -o gen-out/zisk_chain_compute_total_basefee

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" basefees="$2" exp_status="$3" exp_total="$4"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_compute_total_basefee_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_compute_total_basefee_${name}.output"
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_compute_total_basefee.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_compute_total_basefee_${name}.emu.log" 2>&1 || true
...
run_case "three_inc"    "[1000000000,2000000000,3000000000]" 0 6000000000 || FAILED=1
run_case "five_small"   "[100,200,300,400,500]"         0 1500       || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_compute_total_basefee sums basefee across N headers"
  exit 0
else
  echo "==> FAIL"
  exit 1
