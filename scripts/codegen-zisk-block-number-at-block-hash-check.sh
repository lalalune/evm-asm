#!/usr/bin/env bash
# codegen-zisk-block-number-at-block-hash-check.sh
#
# Hash-keyed block.number lookup. Composes K19 + K233.
#
# Output (16 bytes):
...
echo "==> emit zisk_block_number_at_block_hash ELF"
lake exe codegen --program zisk_block_number_at_block_hash \
  --halt linux93 \
  -o gen-out/zisk_block_number_at_block_hash
...
if mode == 'small_number':
    h = encode_header(42)
    witness_headers = build_ssz_section([h])
    block_hash = k256(h)
    expected = struct.pack('<Q', 0) + struct.pack('<Q', 42)
elif mode == 'big_number':
    n = 10_000_000
    h = encode_header(n)
    witness_headers = build_ssz_section([h])
    block_hash = k256(h)
...
elif mode == 'second_header_lookup':
    h0 = encode_header(100)
    h1 = encode_header(101)
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_block_number_at_block_hash.elf \
...
FAILED=0
run_case "small_number_42"             small_number || FAILED=1
run_case "big_number_10M"              big_number || FAILED=1
run_case "genesis_block_zero"          genesis_zero || FAILED=1
run_case "second_header_in_section"    second_header_lookup || FAILED=1
run_case "block_hash_miss"             block_hash_miss || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: block_number_at_block_hash end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
