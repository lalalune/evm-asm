#!/usr/bin/env bash
# codegen-zisk-chain-extract-excess-blob-gas-first-last-check.sh -- PR-K271.
set -euo pipefail

cd "$(dirname "$0")/.."

ZISKEMU="${ZISKEMU:-}"
if [[ -z "$ZISKEMU" ]]; then
  if command -v ziskemu >/dev/null 2>&1; then
    ZISKEMU="$(command -v ziskemu)"
  elif [[ -x "$HOME/.zisk/bin/ziskemu" ]]; then
    ZISKEMU="$HOME/.zisk/bin/ziskemu"
  else
    echo "ziskemu not found -- install via ziskup or set ZISKEMU=..." >&2
    exit 1
  fi
fi

mkdir -p gen-out

echo "==> lake build codegen"
lake build codegen

echo "==> emit zisk_chain_extract_excess_blob_gas_first_last ELF"
lake exe codegen --program zisk_chain_extract_excess_blob_gas_first_last --halt linux93 \
  -o gen-out/zisk_chain_extract_excess_blob_gas_first_last

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" excess_blob_gases="$2" exp_status="$3" exp_first="$4" exp_last="$5"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_extract_excess_blob_gas_first_last_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_extract_excess_blob_gas_first_last_${name}.output"

  uv run --directory execution-specs --quiet python3 -c "
import struct, sys, rlp
def u_be(n):
    if n == 0: return b''
    return n.to_bytes((n.bit_length() + 7) // 8, 'big')

def make_header(excess_blob_gas):
    # 20-field Cancun-shape header: fields 0..19
    return rlp.encode([
        b'\\xa1'*32, b'\\xa2'*32, b'\\xa3'*20, b'\\xa4'*32, b'\\xa5'*32,
        b'\\xa6'*32, b'\\x00'*256, b'', b'\\x01', b'\\x83\\xff\\xff\\xff',
        b'', b'\\x83\\x01\\x02\\x03', b'', b'\\xa7'*32, b'\\x00'*8,
        b'\\x84\\x05\\xf5\\xe1\\x00', b'\\xa8'*32, b'',
        u_be(excess_blob_gas), b'\\xa9'*32,
    ])

vals = $excess_blob_gases
headers = [make_header(v) for v in vals]
N = len(headers)
lengths = b''.join(struct.pack('<Q', len(h)) for h in headers)
flat = b''.join(headers)
with open(sys.argv[1], 'wb') as f:
    record = struct.pack('<Q', N) + lengths + flat
    f.write(record)
    pad = (-len(record)) % 8
    if pad: f.write(b'\x00' * pad)
" "$in_file"

  "$ZISKEMU" -e gen-out/zisk_chain_extract_excess_blob_gas_first_last.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_extract_excess_blob_gas_first_last_${name}.emu.log" 2>&1 || true

  local s_le; s_le="$(dd if="$out_file" bs=1 skip=0  count=8 2>/dev/null | xxd -p | tr -d '\n')"
  local f_le; f_le="$(dd if="$out_file" bs=1 skip=8  count=8 2>/dev/null | xxd -p | tr -d '\n')"
  local l_le; l_le="$(dd if="$out_file" bs=1 skip=16 count=8 2>/dev/null | xxd -p | tr -d '\n')"
  local status first last
  status="$(python3 -c "import struct; print(struct.unpack('<Q', bytes.fromhex('$s_le'))[0])")"
  first="$( python3 -c "import struct; print(struct.unpack('<Q', bytes.fromhex('$f_le'))[0])")"
  last="$(  python3 -c "import struct; print(struct.unpack('<Q', bytes.fromhex('$l_le'))[0])")"

  if [[ "$status" == "$exp_status" && "$first" == "$exp_first" && "$last" == "$exp_last" ]]; then
    printf "  %-26s OK   status=%s first=%s last=%s\n" "$name" "$status" "$first" "$last"
    return 0
  else
    printf "  %-26s FAIL status=%s/%s first=%s/%s last=%s/%s\n" "$name" "$status" "$exp_status" "$first" "$exp_first" "$last" "$exp_last"
    return 1
  fi
}

FAILED=0
run_case "empty"          "[]"                    1 0 0          || FAILED=1
run_case "single_zero"    "[0]"                   0 0 0          || FAILED=1
run_case "single_nonzero" "[7864320]"             0 7864320 7864320 || FAILED=1
run_case "three_inc"      "[100, 200, 300]"       0 100 300      || FAILED=1
run_case "three_dec"      "[300, 200, 100]"       0 300 100      || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_extract_excess_blob_gas_first_last returns (first, last) across N"
  exit 0
else
  echo "==> FAIL"
  exit 1
fi
