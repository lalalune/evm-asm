...
# Non-empty execution_payload: parent_hash = 0xAA*32 (leaf 0,
# Bytes32 inline). Drives the implementation past the
# current `node_0_7 = sha256(ssz_zero_hash[2] || node_4_7)`
# step: ssz_zero_hash[2] (= default node_0_3) is replaced
# with a dynamic computation
#   node_0_1 = sha256(leaf_0=parent_hash || ssz_zero_hash[0])
#   node_0_3 = sha256(node_0_1 || ssz_zero_hash[1])
# Both siblings on the path collapse to existing ssz_zero_hashes
# entries (default fee_recipient = zero -> leaf_1 =
...
# Non-empty execution_payload: block_hash = 0x33*32 (leaf 12,
# Bytes32 inline). Replaces the static `npr_node_12_15`
# constant with 2 dynamic sha256 calls:
#   node_12_13 = sha256(leaf_12=block_hash || npr_leaf_13_transactions_root)
#   node_12_15 = sha256(node_12_13 || npr_node_14_15)
# Two new constants (`npr_leaf_13_transactions_root` for
# the empty transactions list root, `npr_node_14_15` for
...
# Non-empty execution_payload: logs_bloom chunk 0 = 0xAA*32
# (bytes [0..32) of the 256-byte ByteVector; chunks 1..7
# stay default zero). The full logs_bloom merkle is a
# 3-level tree over 8 chunks; supporting chunk 0 alone
# requires 3 sha256 calls with sibling constants that all
# collapse to existing `ssz_zero_hashes` entries
# (ssz_zero_hash[0] for the leaf-chunk-1 sibling,
# ssz_zero_hash[1] for the node_0_1 level-1 sibling,
# ssz_zero_hash[2] for the node_0_3 level-2 sibling).
# Zero new constants. Replaces the prior static
# `npr_leaf_4_logs_bloom_root` with a dynamic computation.
run_fixture "chain1_npr_exec_payload_logs_bloom_chunk0_aa" 1 0 ""        ""                  ""    ""           ""           ""    ""                       ""                                ""    ""    ""    ""    ""                                ""    ""    ""    ""    ""                                  ""    ""    ""    "$(printf 'aa%.0s' {1..32})" || fail=1
