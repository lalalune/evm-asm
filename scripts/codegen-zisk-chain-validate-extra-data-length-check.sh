#!/usr/bin/env bash
# codegen-zisk-chain-validate-extra-data-length-check.sh -- PR-K291.
set -euo pipefail
...
echo "==> emit zisk_chain_validate_extra_data_length ELF"
lake exe codegen --program zisk_chain_validate_extra_data_length --halt linux93 \
  -o gen-out/zisk_chain_validate_extra_data_length

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" ed_lens="$2" exp_status="$3" exp_valid="$4" exp_bad_index="$5"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_validate_extra_data_length_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_validate_extra_data_length_${name}.output"

  uv run --directory execution-specs --quiet python3 -c "
import struct, sys, rlp
def make_header(ed_len):
    ed = b'\\x77' * ed_len if ed_len > 0 else b''
    return rlp.encode([
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_validate_extra_data_length.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_validate_extra_data_length_${name}.emu.log" 2>&1 || true
...
run_case "single_at_32"       "[32]"                     0 1 0 || FAILED=1
run_case "single_at_33"       "[33]"                     0 0 0 || FAILED=1
run_case "single_at_64"       "[64]"                     0 0 0 || FAILED=1
run_case "three_all_under"    "[0, 5, 32]"               0 1 0 || FAILED=1
run_case "three_bad_at_1"     "[10, 50, 20]"             0 0 1 || FAILED=1
run_case "five_bad_at_3"      "[5, 10, 15, 100, 20]"     0 0 3 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_validate_extra_data_length enforces 32-byte cap"
  exit 0
else
  echo "==> FAIL"
  exit 1
