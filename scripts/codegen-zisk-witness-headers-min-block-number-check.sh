#!/usr/bin/env bash
# codegen-zisk-witness-headers-min-block-number-check.sh
#
# Walk witness.headers and compute min(headers[i].number).
# Useful for stateless guests to know the BLOCKHASH window's
# lower bound up-front.
#
# Output (24 bytes):
...
echo "==> emit zisk_witness_headers_min_block_number ELF"
lake exe codegen --program zisk_witness_headers_min_block_number \
  --halt linux93 \
  -o gen-out/zisk_witness_headers_min_block_number
...
#   empty
#   one <number>
#   monotonic <numbers_csv>
#   parse_fail_at <bad_idx>
run_case() {
  local name="$1"; shift
  local mode="$1"; shift

  local in_file="$REPO_ROOT/gen-out/zisk_whmbn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_whmbn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_whmbn_${name}.expected"
...
if mode == 'empty':
    ...
    elements[bad_idx] = b'\\x00'                 # garbage header
    expected_status = 2
    # On parse-fail the min output stays at the pre-fill sentinel
    # MAX_U64; the function only commits the running min on the
    # success path. This is intentional -- callers should treat the
    # min as meaningful only when status == 0.
    expected_min = MAX64
    expected_n = bad_idx
else:
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_witness_headers_min_block_number.elf \
    -i "$in_file" -o "$out_file" -n 4000000 \
    >"$REPO_ROOT/gen-out/zisk_whmbn_${name}.emu.log" 2>&1 || true
