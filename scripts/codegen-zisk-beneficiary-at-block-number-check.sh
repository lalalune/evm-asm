#!/usr/bin/env bash
# codegen-zisk-beneficiary-at-block-number-check.sh
#
# Number-keyed header.beneficiary extractor.
#
# Output (28 bytes):
...
echo "==> emit zisk_beneficiary_at_block_number ELF"
lake exe codegen --program zisk_beneficiary_at_block_number \
  --halt linux93 \
  -o gen-out/zisk_beneficiary_at_block_number
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_bnbn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_bnbn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_bnbn_${name}.expected"
...
def encode_header(number_val, beneficiary):
    fields = [
        b'\\x11'*32, b'\\x22'*32,
        beneficiary,                              # idx 2: beneficiary (20 B)
        b'\\x44'*32, b'\\x55'*32,
...
if mode == 'mev_proposer':
    # Flashbots fee recipient (canonical example pattern)
    beneficiary = bytes.fromhex('dafea492d9c6733ae3d56b7ed1adb60692c98bc5')
    h0 = encode_header(target, beneficiary)
    ...
    h0 = encode_header(100, beneficiary)
    witness_headers = build_ssz_section([h0])
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_beneficiary_at_block_number.elf \
    -i "$in_file" -o "$out_file" -n 4000000 \
    >"$REPO_ROOT/gen-out/zisk_bnbn_${name}.emu.log" 2>&1 || true
...
FAILED=0
run_case "mev_proposer_address"      mev_proposer 101 || FAILED=1
run_case "zero_address"              zero_address 101 || FAILED=1
run_case "pick_second_of_two"        pick_second_of_two 101 || FAILED=1
run_case "number_not_in_section"     number_miss 999 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: beneficiary_at_block_number end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
