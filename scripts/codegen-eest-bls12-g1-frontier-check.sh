#!/usr/bin/env bash
# codegen-eest-bls12-g1-frontier-check.sh -- BLS12 G1 ADD/MSM EEST frontier.
#
# The default filters target the EIP-2537 G1 ADD and G1 MSM fixture families.
...
STEPS="${EEST_BLS12_G1_STEPS:-${EEST_STEPS:-200000000}}"
RUN_DIR="${EEST_BLS12_G1_RUN_DIR:-gen-out/eest-bls12-g1-frontier}"
FX="${EEST_FIXTURES_DIR:-$(pwd)/gen-out/eest-fixtures/$TAG/fixtures/fixtures}"
SKIP="${EEST_BLS12_G1_SKIP:-0}"
LIMIT_OVERRIDE="${EEST_BLS12_G1_LIMIT:-}"
MAX_FAILURES="${EEST_BLS12_G1_MAX_FAILURES:-1}"
REQUIRE_FULL="${EEST_BLS12_G1_REQUIRE_FULL:-0}"
ALLOW_EMPTY="${EEST_BLS12_G1_ALLOW_EMPTY:-0}"
FILTERS=()
EXTRA_ARGS=()
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/codegen-eest-bls12-g1-frontier-check.sh [options] [-- extra harness args]

Options:
  --filter SUBSTR              add a fixture path substring filter
                               (default: bls12_g1add and bls12_g1msm, or
                               words from $EEST_BLS12_G1_FILTERS)
...
count_filter() {
  local filter="$1"
  local count_dir
  count_dir="$(pwd)/gen-out/eest-bls12-g1-count-$(echo "$filter" | tr -c 'A-Za-z0-9._-' '_')"
  rm -rf "$count_dir"
  mkdir -p "$count_dir"
...
if [[ "${#FILTERS[@]}" -eq 0 ]]; then
  if [[ -n "${EEST_BLS12_G1_FILTERS:-}" ]]; then
    read -r -a FILTERS <<< "$EEST_BLS12_G1_FILTERS"
  else
    FILTERS=("bls12_g1add" "bls12_g1msm")
  fi
fi
...
for filter in "${FILTERS[@]}"; do
  ...
  if [[ "$count" -eq 0 ]]; then
    echo "==> BLS12 G1 filter=$filter selected=0 (skipping)"
    continue
  fi
  if [[ "$SKIP" -ge "$count" ]]; then
    echo "==> BLS12 G1 filter=$filter selected=$count skip=$SKIP leaves 0 row(s) (skipping)"
    continue
  fi
  ...
  echo "==> BLS12 G1 filter=$filter count=$count skip=$SKIP limit=$limit require_full=$REQUIRE_FULL"
...
if [[ "$ran_filters" -eq 0 ]]; then
  msg="no BLS12 G1 stateless blocks selected in $FX"
...
echo "==> PASS: BLS12 G1 EEST frontier probe completed filters=$ran_filters selected=$selected_total"
