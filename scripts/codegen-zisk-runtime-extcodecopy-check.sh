#!/usr/bin/env bash
# codegen-zisk-runtime-extcodecopy-check.sh
#
# Exercise opcode 0x3c through the runtime dispatcher with the optional
# account-witness trailer populated by pack-bytecode.py. The fixture builder
# mirrors the standalone EXTCODECOPY context probe, then feeds real EVM
# bytecode: PUSH size/code_offset/memory_offset/address; EXTCODECOPY; RETURN.
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-gen-out/runtime_extcodecopy}"
case "$RUN_DIR" in
  /*) ;;
  *) RUN_DIR="$PWD/$RUN_DIR" ;;
esac
...
def push1(n):
    if not 0 <= n <= 0xff:
        raise ValueError('test immediates must fit PUSH1')
    return [0x60, n]

def extcodecopy_return_bytecode(address, memory_offset, code_offset, length):
    return (
        push1(length) +
        push1(code_offset) +
        push1(memory_offset) +
...
bytecode = extcodecopy_return_bytecode(lookup, 0, code_offset, length)
returned = (expected + bytes(32))[:32]
...
out.joinpath('expected.bin').write_bytes(returned + struct.pack('<Q', 1))
...
if [[ "$FAILED" -ne 0 ]]; then
  echo "==> FAIL: runtime EXTCODECOPY opcode" >&2
  exit 1
fi

echo "==> PASS: runtime EXTCODECOPY opcode"
