#!/usr/bin/env bash
# codegen-zisk-extcodecopy-at-block-number-address-check.sh
#
# Number-keyed EXTCODECOPY. Completes the block_number EVM
# opcode family (extcodesize / extcodehash / sload / extcodecopy).
#
# Output layout:
...
echo "==> emit zisk_extcodecopy_at_block_number_address ELF"
lake exe codegen --program zisk_extcodecopy_at_block_number_address \
  --halt linux93 \
  -o gen-out/zisk_extcodecopy_at_block_number_address
...
run_case() {
  ...
  local in_file="$REPO_ROOT/gen-out/zisk_eccpbn_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_eccpbn_${name}.output"
  local exp_file="$REPO_ROOT/gen-out/zisk_eccpbn_${name}.expected"
...
if mode == 'contract':
    ...
    witness_codes = b''
    addr = ALICE
    expected = struct.pack('<Q', 6) + struct.pack('<Q', 0) + bytes(length)
    ...
    witness_codes = build_ssz_section([code])
    addr = ALICE
    expected = struct.pack('<Q', 7) + struct.pack('<Q', 0)
else:
...
" "$in_file" "$exp_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_extcodecopy_at_block_number_address.elf \
    -i "$in_file" -o "$out_file" -n 6000000 \
    >"$REPO_ROOT/gen-out/zisk_eccpbn_${name}.emu.log" 2>&1 || true
...
FAILED=0
parts="60006000016000526000601a526001601aF3|0|18"   run_case "small_code_full"          contract 101 || FAILED=1
parts="60006000016000526000601a526001601aF3|4|8"    run_case "small_code_mid_window"    contract 101 || FAILED=1
parts="60006000016000526000601a526001601aF3|14|16"  run_case "zero_pad_past_end"        contract 101 || FAILED=1
parts="6000|100|8"                                   run_case "offset_far_past_end"      contract 101 || FAILED=1
parts="0|16"                                         run_case "missing_account_zeros"    missing 101 || FAILED=1
parts="5|32"                                         run_case "empty_code_zeros"         empty_code 101 || FAILED=1
parts="6000|0|4"                                     run_case "integrity_violation"      integrity_violation 101 || FAILED=1
parts="6000|0|8"                                     run_case "number_not_in_section"    number_miss 999 || FAILED=1
parts="6000|0"                                       run_case "too_long_257"             too_long 101 || FAILED=1

echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: extcodecopy_at_block_number_address end-to-end"
  exit 0
else
  echo "==> FAIL"
  exit 1
