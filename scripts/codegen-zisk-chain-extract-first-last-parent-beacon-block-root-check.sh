#!/usr/bin/env bash
# codegen-zisk-chain-extract-first-last-parent-beacon-block-root-check.sh -- PR-K282.
set -euo pipefail
...
echo "==> emit zisk_chain_extract_first_last_parent_beacon_block_root ELF"
lake exe codegen --program zisk_chain_extract_first_last_parent_beacon_block_root --halt linux93 \
  -o gen-out/zisk_chain_extract_first_last_parent_beacon_block_root

REPO_ROOT="$(pwd)"

run_case() {
  local name="$1" pbbrs_list="$2" exp_status="$3" exp_first_hex="$4" exp_last_hex="$5"

  local in_file="$REPO_ROOT/gen-out/zisk_chain_extract_first_last_parent_beacon_block_root_${name}.input"
  local out_file="$REPO_ROOT/gen-out/zisk_chain_extract_first_last_parent_beacon_block_root_${name}.output"

  uv run --directory execution-specs --quiet python3 -c "
import struct, sys, rlp
pbbrs = $pbbrs_list  # list of 32-byte hex strings
def make_header(pbbr_hex):
    pbbr = bytes.fromhex(pbbr_hex)
    return rlp.encode([
...
headers = [make_header(p) for p in pbbrs]
N = len(headers)
...
" "$in_file"
  ...
  "$ZISKEMU" -e gen-out/zisk_chain_extract_first_last_parent_beacon_block_root.elf \
    -i "$in_file" -o "$out_file" -n 5000000 \
    >"$REPO_ROOT/gen-out/zisk_chain_extract_first_last_parent_beacon_block_root_${name}.emu.log" 2>&1 || true
...
A32="$(python3 -c "print('aa'*32)")"
B32="$(python3 -c "print('bb'*32)")"
C32="$(python3 -c "print('cc'*32)")"
...
echo
if [[ $FAILED -eq 0 ]]; then
  echo "==> PASS: chain_extract_first_last_parent_beacon_block_root returns (first, last)"
  exit 0
else
  echo "==> FAIL"
  exit 1
