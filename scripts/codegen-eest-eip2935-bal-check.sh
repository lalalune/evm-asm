#!/usr/bin/env bash
# codegen-eest-eip2935-bal-check.sh -- EIP-7928/EIP-2935 BAL regression.
#
# The default selection covers every stateless row under the EIP-2935 BAL
...
JOBS="${EEST_EIP2935_BAL_JOBS:-${EEST_JOBS:-3}}"
STEPS="${EEST_EIP2935_BAL_STEPS:-${EEST_STEPS:-200000000}}"
RUN_DIR="${EEST_EIP2935_BAL_RUN_DIR:-gen-out/eest-eip2935-bal}"
FX="${EEST_FIXTURES_DIR:-$(pwd)/gen-out/eest-fixtures/$TAG/fixtures/fixtures}"
FILTER="${EEST_EIP2935_BAL_FILTER:-block_access_lists_eip2935}"
LIMIT_OVERRIDE="${EEST_EIP2935_BAL_LIMIT:-}"

[[ -d "$FX" ]] || { echo "fixtures not found at $FX (run scripts/eest-fetch-fixtures.sh '$TAG')" >&2; exit 1; }

count_dir="$(pwd)/gen-out/eest-eip2935-bal-count"
rm -rf "$count_dir"
mkdir -p "$count_dir"
...
while IFS=$'\t' read -r label input expected_hex succ_bit input_len gas_limit relpath; do
  ...
  if IFS=$'\t' read -r status detail < "$result"; then
    if [[ "$status" == "OK" && "${detail:0:210}" == "${expected_hex:0:210}" ]]; then
      ...
      errors=$((errors + 1))
      echo "EIP-2935 BAL error for $relpath: $status $detail" >&2
    else
      semantic_failures=$((semantic_failures + 1))
      echo "EIP-2935 BAL mismatch for $relpath: status=$status" >&2
    fi
  fi
done < "$RUN_MANIFEST"

if [[ "$selected" -eq 0 ]]; then
  echo "no EIP-2935 BAL rows selected" >&2
  exit 1
fi
if [[ "$missing_results" -ne 0 ]]; then
  echo "missing $missing_results EIP-2935 BAL result file(s)" >&2
  exit 1
fi
if [[ "$errors" -ne 0 ]]; then
  echo "found $errors EIP-2935 BAL error row(s)" >&2
  exit 1
fi
if [[ "$semantic_failures" -ne 0 ]]; then
  echo "found $semantic_failures EIP-2935 BAL semantic mismatch row(s)" >&2
  exit 1
fi
if [[ "$ok_full" -ne "$selected" ]]; then
  echo "only $ok_full of $selected EIP-2935 BAL row(s) full-matched" >&2
  exit 1
fi

echo "==> PASS: EIP-2935 BAL rows full-match selected=$selected full=$ok_full"
